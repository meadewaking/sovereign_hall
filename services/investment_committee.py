...
class InvestmentCommittee:
    ...
    async def hold_meeting(
        ...
        defense_task = f"""
提案人{proposer.persona.name}，你的提案受到了以下质询，请只回应会改变结论的问题：

{all_challenges}

【要求】
1. 不复述质询原文，只写回应
2. 有证据才反驳；无证据时承认并降低仓位/置信度
3. 如果需要，调整你的建议仓位或止损线
4. 可以充分展开会改变最终裁决的证据、反证和仓位修正
...
{{
    ...
    "rationale": "决策理由，写清关键证据、反证、否决条件和仓位修正逻辑",
    "voting": {{
        "cio_vote": "approve",
...
3. 质疑乐观假设的合理性
4. 给出具体的反对理由
5. 证据不足时明确写“证据不足”；可充分展开关键反证和压力测试
...
3. 评估当前的市场流动性环境
4. 考虑汇率和国际市场影响（如适用）
5. 无直接影响时写“宏观无新增约束”；有影响时可充分展开情景分支
